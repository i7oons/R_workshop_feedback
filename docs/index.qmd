...
This workshop was supported by the Small Scale Research Grant (SSRG), an initiative of the PIECEs project (funded by National Institute for Health and Care Research (NIHR)) under the mental health programme of Interactive Research & Development (IRD) - Pakistan. PIECEs stands for Improving outcomes for people with psychosis in Pakistan and India -- enhancing the Effectiveness of Community-based care. The initiative aims to enhance research and grant management skills of early-career researchers in Pakistan. Five projects out of 111 applications were selected. This workshop is part of the capacity building effort for SSRG awardees.

In addition to SSRG members, IRD staff members, including fellows of our Grant Writing and Research fellowship, across four IRD country affiliates -- Pakistan, Bangladesh, South Africa, and Philippines -- were also invited to participate in the workshop.
...
This is a five-day online workshop aiming to familiarise the participants with [R language](https://cran.r-project.org/) and [Quarto](https://quarto.org/), an open-source scientific and technical publishing system for creating dynamic documents assembling the narrative, code and its output in one document. The table below shows the topics covered in the workshop and the amount of time spent for each topic. In addition to R syntax, we also covered popular R packages for performing basic data wrangling (primarily [`{dplyr}`](https://dplyr.tidyverse.org/)) and visualisation ([`{ggplot2}`](https://ggplot2.tidyverse.org/)). In addition, we used an example of a simple monthly report of a public health project to show case the basic capabilities of Quarto.
...
participants |> 
  ...
  tbl_summary(label = list(
    ...
    org ~ "Participant's organisation"
  )) |> 
    modify_table_styling(
    columns = label,
    rows = label == "SSRG",
    footnote = "Small Scale Research Grant"
  ) |> 
  as_gt() %>%
  tab_header(title = md("**Participant Characteristics**"))
...
participants |> 
  ...
  as_gt() %>%
  tab_header(title = md("**Respondent Characteristics**"))
```
